...
namespace Gymbross
{
    ...
    internal class Backbone
    {
        ...
        {
            if (MemberName == "" || PhoneNum == "" || Age == "" || Gender == "")
            {
        ...
        {     //Search button
            ...
            if (!string.IsNullOrEmpty(searchQuery))
            {
                try
                {
                    string query = "SELECT memberid, coachid, membername, contactinfo, age, gender FROM member WHERE membername LIKE @SearchQuery";
        ...
        public void Delete()
        {
            ...
            {
                Func.OpenCon();
                string query = $"DELETE FROM member WHERE memberid = '{SharedVariable.memid}';";
                MySqlCommand cmd = new MySqlCommand(query, Func.GetCon());
                cmd.ExecuteNonQuery();
        ...
        public void DisplayMemberData(DataGridView dataGridView)
        {
            try
            {
                ...
                string query = "SELECT memberid, coachid, membername, contactinfo, gender, age FROM member";
                MySqlCommand cmd = new MySqlCommand(query, Func.GetCon());
        ...
        public void CombinedReport(DataGridView dataGridView)
        {
            try
            {
                Func.OpenCon();
                ...
                string query = @"
            SELECT 
                m.memberid, m.membername, m.age, m.contactinfo, m.gender, m.coachid,
                t.amount, t.change, t.paymentmethod, t.total
            FROM 
                member AS m
            JOIN 
                transaction AS t ON m.memberid = t.membershipid;";
        ...
        public void DeleteRecord()
        {
            try
            {
                ...
                string query = $"DELETE FROM member WHERE memberid = '{SharedVariable.Rid}';";
